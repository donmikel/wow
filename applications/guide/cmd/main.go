...
import (
	...
	"github.com/donmikel/wow/applications/guide/config"
	"github.com/donmikel/wow/applications/guide/handlers/guide"
)
...
func gracefulMain() exitCode {
	...
	var g run.Group
	{
		...
		{
			g.Add(func() error {
				var tcpServer guide.ServerInterface
				{
					tcpServer = guide.NewTCPServer(cfg.Key,
						func() int {
					...
					serverHistMetric := kitprom.NewHistogramFrom(stdprom.HistogramOpts{
						...
						Help:      "Guide server requests duration in seconds by method.",
					}, []string{"method"})

					tcpServer = guide.NewInstrumentingMiddleware(serverHistMetric, tcpServer)

					guide.RegisterHandlers(tcpServer, s)
				}
				// Listen and serve.
